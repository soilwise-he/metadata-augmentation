...
def translate(tdata: trans):
	...
	for k,v in ReduceISOPairs.items():
		if k == lower(lang_source) or lower(lang_source) in v['code']:
			ls = k
	    if k == lower(lang_target) or lower(lang_target) in v['code']:
			ts = k

	if not (ls and ts and upper(f"{ls}-{ts}") in LanguagePairs):
    ...
    if myrec and len(myrec) > 0: 
        ...
        myrec = insertSQL("harvest.translations",['source','lang_source','lang_target','date_inserted'],(source,ls,ts,datetime.now()))
